...
import { CardDesignsBackendResponse, CardDesignsResponse } from '@tia/shared/models/cards/card-design.model';
...
import {
  CreateCardRequest,
  CreateCardResponse,
} from '@tia/shared/models/cards/create-card-request.model';
...
getCardDesigns(): Observable<CardDesignsResponse> {
  return this.http.get<CardDesignsBackendResponse>(`${this.apiUrl}/designs`).pipe(
    map((designs) =>
      designs.map((design) => ({
        id: design.design,
