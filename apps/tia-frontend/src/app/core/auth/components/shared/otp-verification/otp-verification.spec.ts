...
describe('OtpVerification', () => {
  ...
  it('should handle login logic (console log) when path is sign-in', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith("LOGIN Logic");
  });
  ...
  it('should set error message signal when API call fails', () => {
    createComponent('sign-up');
    const errorResponse = 'Invalid OTP code';
    vi.spyOn(authService, 'verifyOtpCode').mockReturnValue(throwError(() => errorResponse));
